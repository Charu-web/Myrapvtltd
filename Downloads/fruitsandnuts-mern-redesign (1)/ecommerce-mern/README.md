# Fruits & Nuts — MERN E-Commerce Platform
...
## Phase 1 changes (rebrand + real catalog)

This started as a generic scaffold and has been converted to the Fruits & Nuts brand:

- Brand colors, fonts (Playfair Display + Inter) and copy pulled from the real fruitsandnuts.co.in site export.
- Real logo, hero banner, and product photos wired in (served from `backend/public/uploads`, see `server.js`).
- `backend/utils/seeder.js` now seeds the real product catalog — 6 dry-fruits/nuts products with weight
  variants (250g/500g/1kg), plus category structure for Chocolates (subcategories only — no chocolate
  product photos were available yet, so no fake products were seeded there).
- Navbar, Footer, Home, Contact and a new About page use real brand contact info, social links, outlets,
  and FSSAI registration number.
- `SERVER_URL` added to `backend/.env.example` — used to build the seeded image URLs.

Run `npm run seed` in `backend/` after setting up your `.env` (see below) to load the real catalog.
