...
import { Search, Headphones, Bell, Settings, UserCircle2, Menu, Rocket } from 'lucide-react'
...
export default function Header({ onSaveDraft, onLaunch, draftLoading, launchLoading }) {
  ...
  return (
    <header className="flex items-center justify-between gap-4 py-4">
      ...
      <div className="flex items-center gap-2 pr-4 sm:gap-3 md:pr-8">
        <div className="hidden items-center gap-3 lg:flex">
          {ICON_ACTIONS.map(({ label, icon: Icon }) => (
            ...
            >
              ...
              <span className="hidden text-[9px] font-medium leading-none xl:block">{label}</span>
            </button>
          ))}
        </div>

        <Button variant="secondary" onClick={onSaveDraft} loading={draftLoading} className="!bg-slate-100">
          Save Draft
        </Button>
        <Button variant="primary" onClick={onLaunch} loading={launchLoading}>
